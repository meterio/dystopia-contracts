...
export class Deploy {
  ...
  public static async deployVoltFactory(
    signer: SignerWithAddress
  ) {
    return (await Deploy.deployContract(
      signer,
  ...
  public static async deployDex(
    ...
    const router = await Deploy.deployVoltRouter01(
      signer,
      baseFactory.address,
      networkToken
    );
  ...
  public static async deployVoltSystem(
    ...
    const voter = await Deploy.deployVoltVoter(
      signer,
      ve.address,
      baseFactory,
    ...
    const minter = await Deploy.deployVoltMinter(
      signer,
      ve.address,
      controller.address
    );
    ...
    for (let i = 0; i < initialHolder.length; i++) {
      await Misc.runAndWait(() =>
        token.mint(initialHolder[i], parseUnits(initialAmount))
      );
    }
    ...
    await Misc.runAndWait(() =>
      minter.grantRole(ethers.constants.HashZero, voter.address)
    );
  ...
  public static async updateVoltSystem(
    ...
    return [veDist, gaugesFactory, bribesFactory, voter, minter];
  }
  ...
  public static async deployUpgradeableSystem(
    ...
    const controllerImpl = (await Deploy.deployContract(
      deployer,
      "ControllerUpgradeable"
    )) as ControllerUpgradeable;
    const controllerProxy = await Deploy.deployContract(
      deployer,
    ...
    const veImpl = (await Deploy.deployContract(
      deployer,
      "VeUpgradeable"
    )) as VeUpgradeable;
    const veProxy = await Deploy.deployContract(
      deployer,
      "TransparentUpgradeableProxy",
      veImpl.address,
      deployer.address,
      veImpl.interface.encodeFunctionData("initialize", [
        tokenAddr,
    ...
    const veDistImpl = (await Deploy.deployContract(
      deployer,
      "VeDistUpgradeable"
    )) as VeDistUpgradeable;
    const veDistProxy = (await Deploy.deployContract(
      deployer,
      "TransparentUpgradeableProxy",
      veDistImpl.address,
      deployer.address,
      veDistImpl.interface.encodeFunctionData("initialize", [
        veProxy.address,
        admin.address,
      ])
    )) as VeDistUpgradeable;
    const voterImpl = (await Deploy.deployContract(
      deployer,
      "VoltVoterUpgradeable"
    )) as VoltVoterUpgradeable;
    const voterProxy = await Deploy.deployContract(
      deployer,
      "TransparentUpgradeableProxy",
      voterImpl.address,
      deployer.address,
      voterImpl.interface.encodeFunctionData("initialize", [
        veProxy.address,
        baseFactory,
        gaugesFactory.address,
        bribesFactory.address,
    ...
    const minterImpl = (await Deploy.deployContract(
      deployer,
      "MinterUpgradeable"
    )) as MinterUpgradeable;
    const minterProxy = (await Deploy.deployContract(
      deployer,
      "TransparentUpgradeableProxy",
      minterImpl.address,
      deployer.address,
      minterImpl.interface.encodeFunctionData("initialize", [
        veProxy.address,
        controllerProxy.address,
        admin.address,
      ])
