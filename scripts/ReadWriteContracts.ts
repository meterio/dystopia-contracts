...
async function main() {
  ...
  const fragment = factory.interface.fragments;

  const func_name = await selectFunc(fragment);

  const func = factory.interface.getFunction(func_name);
  ...
  const data = factory.interface.encodeFunctionData(func, args);
  ...
  if (func.stateMutability == "view" || func.stateMutability == "pure") {
    ...
    console.log(factory.interface.decodeFunctionResult(func, result));
  }
  ...
  if (func.stateMutability == "payable") {
    transaction.value = await input({
      ...
      validate: (value = "") =>
        BigNumber.from(value).gt(0) || "Pass a valid address value",
    });
  }
