...
export const contracts_config = JSON.parse(
  readFileSync(contracts_json).toString()
);
...
export function getConfig(netConfigName: string): any {
  ...
  if (!existsSync(fullpath)) {
    ...
    writeFileSync(
      fullpath,
      JSON.stringify(JSON.parse(readFileSync(sample_path).toString()))
    );
  }
  return JSON.parse(readFileSync(fullpath).toString());
}
...
export async function deployOrInput(
  ...
  name = implementation
    ? green(name) + "的" + green("Implementation")
    : green(name);
  ...
  if (answer == "deploy") {
    ...
    address = await input({
      ...
      validate: (value = "") =>
        utils.isAddress(value) || "Pass a valid address value",
    });
  }
  if (implementation) {
...
async function getDefaultValue(
  ...
  if (constructorName == "interestRateModel") {
    ...
    for (let i = 0; i < config.InterestRateModel.length; i++) {
      choices.push({
        name: `${green(config.InterestRateModel[i].name)} - ${yellow(
          config.InterestRateModel[i].contract
        )}${
          config.InterestRateModel[i].address == defaultValue
            ? red("(默认)")
            : ""
        }`,
        value: i,
...
export async function deployProxyOrInput(
  ...
  implementation = utils.isAddress(implementation)
    ? implementation
  ...
  if (proxy_answer == "deploy") {
    ...
    config.address = (
      await deployContractV2(
        ...
        network,
        "SumerProxy",
        [implementation, proxyAdmin, data],
        override
      )
    ).address;
